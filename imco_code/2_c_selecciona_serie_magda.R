...
itaee <- read.csv("../itaee.csv", stringsAsFactors = F, colClasses = c("character","character","character","numeric", "numeric"))
x11 <- read.csv("../municipios_x11_input.csv", stringsAsFactors = F,
...
prb <- x11 %>% mutate(CVEENT= substr(CVEMUN,1,2)) %>% select(-CVEMUN) %>%
...
sel_x11 <- prb %>% select(CVEENT, Estado,predictor_x11,corr_x11) %>% unique() %>%
          group_by(CVEENT, Estado) %>%
          arrange(corr_x11)%>%
          mutate(ones = 1,
                 rank = cumsum(ones)) %>% ungroup() %>%
          filter((predictor_x11 =="banamex_x11"& corr_x11>.95 ) | rank == 9 ) %>%
          group_by(CVEENT, Estado) %>%
                     arrange(corr_x11)%>%
                     mutate(ones = 1,
                            rank = cumsum(ones)) %>% ungroup() %>%
...
x11_selecto <- x11 %>% mutate(CVEENT= substr(CVEMUN,1,2)) %>%
              gather(predictor_x11, valor,todos_x11:otrosbbv_x11,convert= T)%>%
              inner_join(sel_x11, by = c("CVEENT", "predictor_x11")) %>% select(-rank,-ones,-Estado,- CVEENT, -corr_x11) %>%
              rename(selecto_x11 = valor)


# después de hacer las pruebas vamos a hacer algunos cambios proque no cuadran en los municipios:
...
write.csv(x11_selecto,"../predictor_x11_selecto_estatal_corregido.csv", row.names = F)
